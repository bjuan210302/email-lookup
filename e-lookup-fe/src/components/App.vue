<template>
  <ErrorBoundary>
    <div class="flex min-w-full min-h-full" @mouseleave="() => toggleDragging(false)"
      @mouseup="() => toggleDragging(false)">

      <!-- LEFT -->
      <div class="max-h-screen overflow-y-auto" :style="{ width: `${dividerPosition}%` }">
          ...
          <div class="flex justify-between items-center">
            ...
            <div class="flex items-center min-w-[50%]">
              ...
              <button class="inline-flex ml-2" @click="oConfigModal">
                <svg class="stroke-zinc-600" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
        ...
        <div class="overflow-y-auto p-3">
          ...
          <div v-if="totalResults === 0" class="absolute top-[50%] left-0 text-center
            my-auto text-zinc-300 font-semibold tracking-wider" :style="{ width: `${dividerPosition}%` }">
            No results found
          </div>
      ...
      <!-- EXPANDED EMAIL-->
      <div class="top-0 min-h-full max-h-screen bg-gray-50 overflow-y-auto border-solid border-l-2 border-gray-200"
        :style="{ width: `${100 - dividerPosition}%` }">

        <span v-if="!selectedEmail._id" class="absolute top-[50%] right-0 text-center
          my-auto text-zinc-400 tracking-wider" :style="{ width: `${100 - dividerPosition}%` }">
...
// Drag
const dividerPosition = ref((3 / 5) * 100)
...
// from https://codepen.io/andkoo/pen/dwgpBN
const handleDragging = (e: MouseEvent) => {
  const percentage = (e.pageX / window.innerWidth) * 100
  const offset = 30
  if (percentage >= offset && percentage <= 100 - offset) {
